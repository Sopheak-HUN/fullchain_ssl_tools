...
const canDownloadAll = computed(() =>
  !!(displayCertPem.value || displayFullchainPem.value || displayPrivkeyPem.value || displayServerPem.value)
)
...
// ─── nginx snippet ────────────────────────────────────────────────────────────
const nginxSnippet = computed(() => `server {
    listen 443 ssl;
...
// ─── Apache snippet ───────────────────────────────────────────────────────────
const apacheSnippet = computed(() => `<VirtualHost *:443>
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <header class="mb-8">
        ...
        <p class="mt-4 max-w-4xl text-slate-300 text-lg leading-8">
          ...
          validate the chain, verify key match, and export configs for Kubernetes, nginx, and Apache.
        </p>
      </header>
      ...
      <main class="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div class="space-y-8">
            ...
            <div>
              ...
              <p class="mt-2 text-slate-300 leading-7">
                Drag & drop or click to upload. If intermediate chain is missing, backend will try to fetch it from AIA URL.
              </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div
                ...
                :class="draggingOver === 'leaf' ? 'border-cyan-500 bg-cyan-500/5' : 'border-slate-800'"
              ...
              >
                <div class="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-slate-400 mt-1">Required — drag & drop or upload</p>
                  </div>
                </div>
                <label class="block">
                  ...
                  <input type="file" accept=".crt,.pem,.cer,text/plain" class="hidden" @change="handleFile($event, 'leaf')" />
                </label>
                <textarea
                  v-model="leafCert"
                  rows="6"
              ...
              <div
                class="rounded-2xl border bg-slate-950 p-4 transition-colors"
                :class="draggingOver === 'key' ? 'border-cyan-500 bg-cyan-500/5' : 'border-slate-800'"
              ...
              >
                <div class="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-slate-400 mt-1">Optional — drag & drop or upload</p>
                  </div>
                </div>
                <label class="block">
                  <span class="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2 text-sm cursor-pointer hover:bg-slate-900 transition">
                  ...
                  <input type="file" accept=".key,.pem,text/plain" class="hidden" @change="handleFile($event, 'key')" />
                </label>
                <textarea
                  v-model="privateKey"
                  rows="6"
            ...
            <div
              class="rounded-2xl border bg-slate-950 p-4 transition-colors"
              :class="draggingOver === 'chain' ? 'border-cyan-500 bg-cyan-500/5' : 'border-slate-800'"
            ...
            >
              <div class="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h3 class="font-semibold text-slate-100">Intermediate chain (.crt / .pem / .ca-bundle)</h3>
                  <p class="text-sm text-slate-400 mt-1">Optional — drag & drop or upload. Backend will try to discover if omitted.</p>
                </div>
              </div>
              <label class="block">
                <span class="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2 text-sm cursor-pointer hover:bg-slate-900 transition">
                ...
                <input type="file" accept=".crt,.pem,.cer,.ca-bundle,text/plain" class="hidden" @change="handleFile($event, 'chain')" />
              </label>
              <textarea
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
              <div>
                ...
                <input v-model="certName" class="w-full rounded-2xl bg-slate-950 border border-slate-700 px-4 py-3 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
              <div>
                <label class="block text-sm font-medium text-slate-200 mb-2">fullchain filename</label>
                <input v-model="fullchainName" class="w-full rounded-2xl bg-slate-950 border border-slate-700 px-4 py-3 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
              <div>
                <label class="block text-sm font-medium text-slate-200 mb-2">privkey filename</label>
                <input v-model="privkeyName" class="w-full rounded-2xl bg-slate-950 border border-slate-700 px-4 py-3 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
              <div>
                <label class="block text-sm font-medium text-slate-200 mb-2">server filename</label>
                <input v-model="combinedName" class="w-full rounded-2xl bg-slate-950 border border-slate-700 px-4 py-3 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-4">
              ...
              <div v-if="errorMessage" class="text-red-300 text-sm">{{ errorMessage }}</div>
            </div>
            ...
            <div v-if="serverResult" class="rounded-2xl border border-slate-800 bg-slate-950 p-4 space-y-3">
              ...
              <!-- Cert info -->
              <div v-if="serverResult.certInfo" class="rounded-xl bg-slate-900 border border-slate-800 p-3 text-sm space-y-2">
                <div class="text-slate-400 font-medium mb-1">Certificate info</div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1.5">
                  <div>
                    <span class="text-slate-400">Subject: </span>
                    <span class="text-slate-100 break-all">{{ serverResult.certInfo.subject }}</span>
                  </div>
                  <div>
                    <span class="text-slate-400">Issuer: </span>
                    <span class="text-slate-100 break-all">{{ serverResult.certInfo.issuer }}</span>
                  </div>
                  <div>
                    <span class="text-slate-400">Valid from: </span>
                    <span class="text-slate-100">{{ formatDate(serverResult.certInfo.notBefore) }}</span>
                  </div>
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="text-slate-400">Expires: </span>
                    <span class="text-slate-100">{{ formatDate(serverResult.certInfo.notAfter) }}</span>
                    <span
                      ...
                      "
                    >
                      {{ serverResult.certInfo.isExpired ? 'Expired' : `${serverResult.certInfo.daysLeft}d left` }}
                    </span>
                  </div>
                  <div class="md:col-span-2">
                    <span class="text-slate-400">SANs: </span>
                    <span class="text-slate-100 break-all">{{ serverResult.certInfo.sans.join(', ') || 'None' }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid md:grid-cols-2 gap-4 text-sm">
                <div class="rounded-xl bg-slate-900 border border-slate-800 p-3">
                  ...
                  <div class="text-slate-100 mt-1">{{ serverResult.chainSource || 'unknown' }}</div>
                </div>
                <div class="rounded-xl bg-slate-900 border border-slate-800 p-3">
                  ...
                  <div
                    class="mt-1"
                    :class="
                      serverResult.keyMatches === null ? 'text-slate-400'
                      : serverResult.keyMatches ? 'text-green-300' : 'text-red-300'
                    "
                  >
              ...
              <div class="rounded-xl bg-slate-900 border border-slate-800 p-3">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <span
                    class="px-2 py-0.5 rounded-full text-xs font-semibold"
                    :class="serverResult.verifyOk ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'"
                  >
            ...
            <div class="rounded-2xl border border-slate-800 bg-slate-950 p-5 space-y-4">
              <div class="flex items-center gap-2">
                ...
                <h3 class="font-semibold text-slate-100">Kubernetes TLS Secret</h3>
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-slate-300 mb-1">Secret name</label>
                  <input
                ...
                <div>
                  <label class="block text-sm font-medium text-slate-300 mb-1">Namespace</label>
                  <input
              ...
              <div class="relative">
                <pre class="rounded-xl bg-slate-900 border border-slate-800 p-4 text-xs text-slate-300 overflow-x-auto whitespace-pre h-[220px] overflow-y-auto">{{ k8sYaml }}</pre>
                <div class="absolute top-3 right-3 flex gap-2">
                  <button
                    ...
                    :class="copiedSnippet === 'k8s' ? 'border-green-500/50 text-green-400' : 'text-slate-300'"
                  >
              ...
              <p class="text-xs text-slate-500">
                Apply with:
                <code class="bg-slate-900 px-1.5 py-0.5 rounded">kubectl apply -f {{ k8sSecretName }}.yaml</code>
              </p>
            </div>
            ...
            <div class="rounded-2xl border border-slate-800 bg-slate-950 p-5 space-y-4">
              <div class="flex items-center gap-2">
                ...
                <h3 class="font-semibold text-slate-100">Web server config snippets</h3>
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-slate-300 mb-1">Certificate path</label>
                  <input
                ...
                <div>
                  <label class="block text-sm font-medium text-slate-300 mb-1">Private key path</label>
                  <input
              ...
              <div>
                <div class="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    class="rounded-lg border border-slate-700 px-2 py-1 text-xs hover:bg-slate-800 transition inline-flex items-center gap-1"
                    :class="copiedSnippet === 'nginx' ? 'border-green-500/50 text-green-400' : 'text-slate-300'"
                  >
                ...
                <pre class="rounded-xl bg-slate-900 border border-slate-800 p-4 text-xs text-slate-300 overflow-x-auto whitespace-pre">{{ nginxSnippet }}</pre>
              </div>
              ...
              <div>
                <div class="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    class="rounded-lg border border-slate-700 px-2 py-1 text-xs hover:bg-slate-800 transition inline-flex items-center gap-1"
                    :class="copiedSnippet === 'apache' ? 'border-green-500/50 text-green-400' : 'text-slate-300'"
                  >
                ...
                <pre class="rounded-xl bg-slate-900 border border-slate-800 p-4 text-xs text-slate-300 overflow-x-auto whitespace-pre">{{ apacheSnippet }}</pre>
              </div>
            </div>
        ...
        <aside class="rounded-3xl border border-slate-800 bg-slate-900 shadow-2xl p-6 space-y-6">
          ...
          <div class="rounded-2xl border border-slate-800 bg-slate-950 p-4 text-sm text-slate-300 space-y-2">
            <h4 class="font-semibold text-slate-100">Drag & drop tips</h4>
            <p>Each input zone accepts drag & drop. Drop directly onto the card — it highlights in cyan when active.</p>
          </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p class="mt-1 text-sm leading-6">
                  ...
                  Auto-fetch improves results but is not guaranteed to fully solve every CA chain.
                </p>
              </div>
            </div>
          </div>
